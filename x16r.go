...
func MinerHacashDiamond(diamondnumber int, loopnum int, stopmark *byte, blockhash []byte, address []byte) ([]byte, string) {
	var nonce [8]C.char
	...
	C.miner_diamond_hash(dmnb, lpnm, (*C.char)((unsafe.Pointer)(stopmark)), tarhash, taraddr, &nonce[0], &diamond[0])
